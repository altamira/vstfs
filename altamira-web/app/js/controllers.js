...
angular.module('myApp.controllers', [])
        ...
        .controller('CreateRequestController', ['$scope', '$window', '$http', function($scope, $window, $http) {
                ...
                var init = function() {
                    ...
                    $scope.cuts = [{id: 0, description: 'Bobina'}, {id: 1, description: 'Chapa'}];
                    $scope.cut = $scope.cuts[0];

                    $scope.laminations = [{id: 'FQ', description: 'Fina Quente'}, {id: 'FF', description: 'Fina Frio'}];
                    $scope.treatments = [{id: 'PR', description: 'Chapa Preta'}, {id: 'DE', description: 'Decapado'}, {id: 'GA', description: 'Galvanizado'}];
                    ...
                    $scope.request = {id: 0, date: $scope.currentDate, creator: '', items: []};
                ...
                $scope.add = function(item) {
                    ...
                    .success(function(result) {
                        if (result.error === 0) {
                            $scope.item.material = result.data;
        ...
        .controller('CreateQuotationController', ['$scope', '$window', '$http', function($scope, $window, $http) {
                ...
                var init = function() {
                    ...
                    $http({
                        ...
                        headers: {'Accept': 'application/json'},
                            ...
                            .error(function() {
                                alert("Não foi possãvel carregar a lista de Normas !");
                            });
                    ...
                    $http({
                        ...
                        headers: {'Content-Type': 'application/json', 'Accept': 'application/json'},
                            ...
                            .error(function() {
                                alert("Não foi possãvel carregar a Cotação " + $location.search()['id'] + " !");
                            });
...
//		    	//var content = ;
// 
//	            $.Dialog.title('Cotaãão ' + supplier.name);
...
//	    quote.standard = "SAE-1008";
                ...
                $scope.remove = function(item) {
                    ...
                    if (confirm) {
                         ...
                         *          no lado do servidor.                                                                *
                         *                                                                                              *
                         *          Outra forma de evitar esse problema ã setar o default header no inãcio do modulo:   *
                         *                                                                                              *
                         *          $http.defaults.headers.common['Content-Type'] = 'application/json';                 *
                         *          $http.defaults.headers.common['Accept'] = 'application/json';                       *
                         *                                                                                              *
                         *          Util se for necessãrio um controle de concorrencia para comparar o valor de cada um *
                         *          dos campos com o registro no banco de dados para evitar apagar algum registro que   *
                         *          tenha sido alterado por outro usuãrio enquanto esta sendo acesso aqui nesta tela    *
                         ***********************************************************************************************/
//			var result $http({
//				method: 'DELETE',
...
//			})
                         ...
                         *          A diferenca ã que não necessita manipular os headers, porque o id do objeto vai ser *
                         *          passado por parametro, neste caso o servidor vai ignorar o conteudo da requisiãão.  *
                         *                                                                                              *
                         *          Util se não for necessãrio verificar o estado do objeto para efeito de controle de  *
                         *          concorrãncia                                                                        *
                         ***********************************************************************************************/
                        ...
                        /************************************************************************************************
                         * Em ambos os casos o servidor retorna o resultado da operaãão em formato JSON:                *
                         *                                                                                              *
                         * Formato da mensagem: {error: 12345, message: 'Alteraãão efetuada com sucesso !'}             *
                         ***********************************************************************************************/
